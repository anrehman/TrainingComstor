...
module.exports = function(controller) {
  controller.hears(["espacio"], "direct_message,direct_mention", function(
    bot,
    message
  ) {
      ...
      convo.ask(question, [
        {
          ...
          callback: function(response, convo) {
            ...
            async.waterfall([
              function(callback) {
                spark.rooms.create({ title: roomName }, function(
                  err,
                  response
                ) {
                  if (err) {
                    convo.say(
                      "I am sorry, there was an error creating the room, Please contact administrator."
                    );
                    convo.next();
                  } else {
              ...
              function(roomId, callback) {
                spark.memberships.list(membershipParameters, function(
                  err,
                  response
                ) {
                  if (err) {
